...
async function main() {
  ...
  const eventsJsonPath = path.resolve(
    process.cwd(),
    process.env.EVENTS_JSON ?? '../asu-guide/data/asu-events.json',
  )
  ...
  for (const record of records) {
    ...
    if (
      !existingEvent ||
      !existingEvent.embedHash ||
      existingEvent.embedHash !== newHash ||
      !existingEvent.embedText
    ) {
  ...
  if (eventsToReembed.length > 0) {
    try {
      ...
      if (error instanceof Error) {
        console.warn(
          `Warning: Failed to embed events due to: ${error.message}. Dense retrieval will be unavailable until seed is re-run with VPN.`,
        )
      } else {
        console.warn(
          'Warning: Failed to embed events due to an unknown error. Dense retrieval will be unavailable until seed is re-run with VPN.',
        )
      }
    }
  }
  ...
  for (const record of records) {
    ...
    ftsRecords.push([row.id, row.title, row.description, row.club, row.type])
  }
