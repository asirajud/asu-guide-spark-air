...
async function airFetch(path: string, body: unknown, timeoutMs: number): Promise<Response> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new AirError(
        `HTTP ${res.status}: ${text.substring(0, 400)}`,
        res.status
      );
    }
...
export async function embed(texts: string[], batchSize = 32): Promise<Float32Array[]> {
  ...
  for (let i = 0; i < texts.length; i += batchSize) {
    ...
    const res = await airFetch('/embeddings', {
      model: EMBED_MODEL,
      input: batch
    }, 60_000);
    ...
    console.log(`[air] embedded ${Math.min(i + batchSize, texts.length)}/${texts.length} in ${ms}ms`);
  }
...
export async function rerank(query: string, documents: string[], topN: number): Promise<RerankHit[]> {
  ...
  const res = await airFetch('/rerank', {
    model: RERANK_MODEL,
    query,
    documents,
