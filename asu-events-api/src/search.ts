...
export function fuse(bm25: ScoredHit[], dense: ScoredHit[], k = RRF_K): FusedHit[] {
  ...
  return Array.from(fusedMap.values())
    ...
    .map((entry) => {
      ...
      const bm25Rank = entry.stages.bm25 !== undefined ? bm25.findIndex(hit => hit.id === entry.id) + 1 : Infinity
      const denseRank = entry.stages.dense !== undefined ? dense.findIndex(hit => hit.id === entry.id) + 1 : Infinity
...
export async function search(query: string, filters: SearchFilters = {}): Promise<SearchResult> {
  ...
  const trace: SearchTrace = {
    ...
    dense: { ms: Math.round(denseMs * 100) / 100, count: denseHits.length, top: [], available: false },
    fusion: { ms: Math.round(fusionMs * 100) / 100, count: fused.length, onlyBm25: 0, onlyDense: 0, both: 0 },
  ...
  const rows = db.select().from(events).where(inArray(events.id, fused.map(f => f.id))).all()
  ...
  if (scores.size > 0) {
    ...
    const rerankedSlice = fusionOrderNoDuplicates.slice(0, 20).map(hit => ({
      ...hit,
  ...
  // Count fusion types
  trace.fusion.onlyBm25 = fused.filter(f => f.stages.bm25 !== undefined && f.stages.dense === undefined).length
  trace.fusion.onlyDense = fused.filter(f => f.stages.bm25 === undefined && f.stages.dense !== undefined).length
  trace.fusion.both = fused.filter(f => f.stages.bm25 !== undefined && f.stages.dense !== undefined).length
