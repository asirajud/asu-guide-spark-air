...
export type JsonRpcResponse = { jsonrpc: '2.0'; id: string | number | null } & (
...
export const LIST_CAPABILITIES: ToolSpec = {
  ...
  description:
    'List every tool this assistant can reach beyond the ones already loaded, with the service that owns each.',
  route: { method: 'GET', path: '/__internal/list_capabilities' },
  inputSchema: { type: 'object', properties: {}, additionalProperties: false },
}

export function sessionTools(): ToolSpec[] {
  return SESSION_TOOLS.map((name) => findTool(name))
    .filter(Boolean)
