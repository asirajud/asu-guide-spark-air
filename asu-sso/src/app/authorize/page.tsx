...
export default async function AuthorizePage({ searchParams }: { searchParams: SearchParams }) {
  ...
  if (!client_id || !client) {
    return <RequestError message={`Unknown client_id "${client_id}". This demo IdP only knows the client registered in clients.json.`} />;
  }

  if (!redirect_uri || !isValidRedirectUri(client, redirect_uri)) {
    return <RequestError message={`redirect_uri "${redirect_uri}" is not registered for this client.`} />;
  }

  if (response_type !== "code") {
    return <RequestError message={`Unsupported response_type "${response_type}". This server only supports the authorization code flow.`} />;
  }
  ...
  if (code_challenge_method !== "S256") {
    return <RequestError message={`Unsupported code_challenge_method "${code_challenge_method}". Only S256 is supported.`} />;
  }
