...
export async function POST(req: NextRequest) {
  ...
  if (!consumed.ok) {
    return fail(400, "invalid_grant", "Authorization code is invalid, expired, or has already been used.");
  }

  // PKCE verification (must happen after code consumption)
  if (!verifyPkce(code_verifier, consumed.record.codeChallenge)) {
    return fail(400, "invalid_grant", "PKCE verification failed: code_verifier does not match code_challenge.");
  }
  ...
  return NextResponse.json(
    {
      ...
      id_token: {
        ...
        _note: "Demo only: this is a plain JSON object, NOT a signed JWT. Do not treat it as verifiable.",
      },
    },
