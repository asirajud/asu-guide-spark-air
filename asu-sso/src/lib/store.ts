...
export function makeUser(asurite: string): UserInfo {
  ...
  const name = normalized.charAt(0).toUpperCase() + normalized.slice(1) + ' (demo user)'
  return {
    sub: 'asu|' + normalized,
    asurite: normalized,
...
export function issueToken(asurite: string, clientId: string, scope: string): TokenRecord {
  sweep()
